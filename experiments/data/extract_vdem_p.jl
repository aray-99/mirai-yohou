...
# raw/<ISO3>_pol.csv(year,value。value = v2cacamps_osp / 4 ∈ [0,1])を生成する。
#
...
#
# 実行: julia --project=experiments experiments/data/extract_vdem_p.jl [ISO3...] [--force]
...
function main()
    ...
    targets = [joinpath(RAW_DIR, "$(c)_pol.csv") for c in countries]
    ...
    for iso in countries
        ...
        sort!(sub, :year)
        csvpath = joinpath(RAW_DIR, "$(iso)_pol.csv")
