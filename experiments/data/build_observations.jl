...
#   (#0030-7)。
#
# 実行(サマリ): julia --project=experiments experiments/data/build_observations.jl
...
_series_path(country, var) = joinpath(OBS_RAW_DIR, "$(country)_$(var).csv")
...
(変数ファイル名, 変換, 状態インデックスまたは :log_y, 暫定 sd, 点別sdキー)。
sd は保持座標単位の暫定値(検証前に DECISIONS で凍結、#0030-7)。
"""
const REAL_SERIES = [
    (var = "P",       kind = :log_norm,  target = IX_P,    sd = 0.005, sdkey = ""),
    (var = "w",       kind = :logit_pct, target = IX_W,    sd = 0.01,  sdkey = ""),
    (var = "y",       kind = :log_y,     target = :log_y,  sd = 0.02,  sdkey = ""),
    (var = "g_swiid", kind = :logit,     target = IX_G,    sd = 0.05,  sdkey = "measurement_se", sdinflate = 2.0),
    (var = "T_proxy", kind = :log_norm,  target = IX_T,    sd = 0.15,  sdkey = ""),
    (var = "phi",     kind = :logit_pct, target = IX_PHI,  sd = 0.05,  sdkey = ""),
    (var = "v",       kind = :log_norm,  target = IX_V,    sd = 0.05,  sdkey = ""),
    (var = "tau",     kind = :logit,     target = IX_TAU,  sd = 0.15,  sdkey = ""),
    (var = "p",       kind = :logit,     target = IX_PP,   sd = 0.10,  sdkey = "measurement_sd", sdinflate = 1.0),
]
...
function _summarize(country)
    params = build_params(country == "JPN" ? :stable : :volatile)
