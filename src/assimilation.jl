...
struct AssimResult
    ...
    # g 計装診断(DECISIONS #0065)。`instrument_g = false`(既定)なら常に空
    # ベクトル(計算自体を行わない、後方互換・判定数値に無関係)。各要素は
    # Dict("t", "phase" [:pre/:post], "update_type"
    # [:count_weekly/:g_swiid_annual/:other_obs], "g_mean", "g_sd",
    # "last_g_swiid_obs")。読み取り専用の診断で、乱数・状態・重みは変更しない。
...
"""
    g_diag_entry(t, phase, update_type, E, last_g_swiid) -> Dict

g 計装診断1レコード(DECISIONS #0065)。`E`(全行 × N メンバー)の `IX_G` 行
から読み取り専用で加重なし平均・標本標準偏差(不偏補正、N>1)を計算する
だけで、`E` そのものは一切変更しない。`last_g_swiid` は直近に観測された
g_swiid 値(`Ref{Union{Nothing,Float64}}`。まだ観測が無ければ `nothing`)。
"""
function g_diag_entry(t::Float64, phase::Symbol, update_type::Symbol,
                      E::AbstractMatrix{Float64},
                      last_g_swiid::Ref{Union{Nothing,Float64}})
    gvals = view(E, IX_G, :)
    n = length(gvals)
    gmean = sum(gvals) / n
    gsd = n > 1 ? sqrt(sum(abs2, gvals .- gmean) / (n - 1)) : 0.0
...
g 計装診断(DECISIONS #0065): `instrument_g = true`(既定 false)にすると、
各週次カウント再重み付け(再抽選の前後、`update_type = :count_weekly`)と
各解析ステップ(`enks_analysis!` 前後、バッチに `:g_swiid` 観測を含めば
`update_type = :g_swiid_annual`、それ以外は `:other_obs`)の前後で
アンサンブルの `g`(`IX_G` 行)の平均・sd と直近の g_swiid 観測値を
`AssimResult.g_diag` に記録する。**読み取り専用**(乱数消費・状態・重みへの
副作用は一切ない)なので、既定 false のときは元コードと完全に同一の計算
(計装のオーバーヘッドすらない)、true でも同化の数値結果は不変。
"""
function run_assimilation(params::ModelParameters, E0::Matrix{Float64},
    ...
    for step in 1:nsteps
        ...
        if step % wsteps == 0
            ...
            instrument_g &&
                push!(g_diag, g_diag_entry(t_next, :pre, :count_weekly, E, last_g_swiid))
            ...
            instrument_g &&
                push!(g_diag, g_diag_entry(t_next, :post, :count_weekly, E, last_g_swiid))
            fill!(Lambda, 0.0)
            window_count = 0
        end
        ...
        if haskey(obs_at, step + 1)
            ...
            if instrument_g
                ...
                push!(g_diag, g_diag_entry(t_next, :pre, utype, E, last_g_swiid))
            end
            ...
            instrument_g &&
                push!(g_diag, g_diag_entry(t_next, :post, utype, E, last_g_swiid))
        end
