...
"""
θ の静的妥当性チェック(#0064): fit_exogenous → ModelParameters 構築が通るか。
forward 実行なしで安価。forward_G の1つ目の try/catch(De レジーム条件等の
違反で nothing を返す経路)と同一の失敗条件を初期化時に前倒しで判定する。
"""
static_theta_valid(regime, theta, recs, window) =
    try
        fit_exogenous(params_from_theta(regime, theta), recs, window)
        true
    catch
        false
    end
...
- `validate_prior`(#0064、M10): `true` なら初期 H の各列を静的妥当性
  チェック(`static_theta_valid`、forward 実行なし)に通し、失敗列は同一
  prior から再ドロー(上限 200 回/列、超過はエラー)。凍結済みの sd・
  prior 中心・J・iters の値は変えず、事実上の切断分布(従来は forward_G が
  無効列を棄却して切断していた)を J 本維持のまま初期化時に適用する。
  既定 `false`(M8/M9 経路の乱数消費・挙動を一切変えない後方互換)。
"""
function calibrate(country::String; J::Int = 16, iters::Int = 3, N::Int = 50,
    ...
    if validate_prior
        # #0064: 初期 H の静的妥当性リサンプリング(prior の切断分布化)。
        # validate_prior=false では本ブロックは rng を消費しない(後方互換)。
        n_resample = 0
        for j in 1:J
            tries = 0
            while !static_theta_valid(ccfg.regime, from_eta(H[:, j]), recs0, window)
                tries += 1
                tries <= 200 || error("validate_prior(#0064): 列 $j が 200 回の" *
                                      "再ドローで静的チェックを通りません")
                H[:, j] = eta0 .+ sd_vec .* randn(rng, length(eta0))
            end
            n_resample += tries
        end
        println("  prior 妥当性リサンプリング(#0064): 再ドロー合計 $n_resample 回")
    end
